...
for w = 60:10:100
    resfilname = "resultw("+num2str(w)+").xlsx";
%     xi = steer*pi/180;
    xi = 0;
    step_phi = 0.0001; %(rad)
            ...
            for group = 1:numgroup %iterate subgroups
                ...
                for delta_phi = 0:step_dphi:2*pi
                    ...
                    if(i==1 && iter==1 && group ==1 && delta_phi==0)
                        ...
                        save("originalw("+num2str(w)+").mat", 'mainlobe');
                        saveas(gcf,"firstw("+num2str(w)+").fig");
                        firstpsll = psll;
    ...
    xlswrite(resfilname,min_psll,"final psll");
    figure(2+w);
    plot(varphi*180/pi, best_mainlobe.^2);
%     hold on
    saveas(gcf,"lastw.fig"');
end
